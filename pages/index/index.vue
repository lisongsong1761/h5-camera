<template>
	<view class="content">
		...
		<view class="my_photo" v-if="photoSrc">
			<image mode="aspectFit" class="camera_photo" :src="photoSrc"></image>
			<image class="cancel_btn" src="@/static/cancel.png" @click="cancel"></image>
			<image class="confirm_btn" src="@/static/confirm.png" @click="uploadFile"></image>
			 <!-- @click="confirm" -->
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			drawFirstCanvas() {				
				...
				if(this.type === 1) {
					...
					if(aspectRatio > A4_aspectRatio) { // 先取宽
						...
						context.moveTo((screenWidth-basicData*0.1867)/2, 44);
						context.lineTo((screenWidth-basicData*0.1867)/2, realHeight);
						context.lineTo((screenWidth-basicData*0.1867)/2 + basicData*0.1867, realHeight);
						context.lineTo((screenWidth-basicData*0.1867)/2 + basicData*0.1867, 44);
						context.closePath();
						...
						context.strokeRect((screenWidth-basicData*0.1867)/2, 44, basicData*0.1867, basicData)
						// 内虚线
						...
						for(let i=0;i<6;i++) {
							context.strokeRect((screenWidth-basicData*0.1867)/2, 44+basicData/6*i, basicData*0.1867, basicData/6)
						}
			...
			takePhoto() { // 添加防抖
				...
				if (this.videoPlaying) {
					...
					for(let i=0,len = pixeldata.length ;i<len;i+=4){
							let black =( imageData.data[i]*0.7 + imageData.data[i+1]*1.1 + imageData.data[i+2]*1.2)/3;
							if(black>=80){
									black = 255
							}else{
									black = 0;
							}
							imageData.data[i] = black;
							imageData.data[i+1] = black;
							imageData.data[i+2] = black;
					} */
			...
			uploadFile() {
				...
				var export_blob = this.dataURLtoFile(this.photoSrc);
